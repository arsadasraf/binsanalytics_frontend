...
export default function EmployeeMaster() {
    ...
    return (
        <div className="space-y-6">
            {/* Header Actions */}
            <div className="flex flex-row justify-between items-center bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-gray-100 gap-3 md:gap-4">
                <div className="relative flex-1 md:flex-none md:w-72">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border-transparent focus:bg-white border focus:border-blue-500 rounded-xl transition-all outline-none text-sm"
                    />
                </div>
                <button
                    onClick={handleOpenAdd}
                    className="flex-none flex items-center justify-center gap-2 px-3 md:px-5 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-200 transition-all font-medium"
                >
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    <div className="col-span-2" ref={dropdownRef}>
                                                        ...
                                                        <div className="relative">
                                                            <div
                                                                className="w-full min-h-[42px] px-3 py-2 bg-gray-50 border-transparent focus-within:bg-white border focus-within:border-blue-500 rounded-lg transition-all outline-none cursor-pointer flex flex-wrap gap-2 items-center"
                                                                onClick={() => { setIsSkillDropdownOpen(true); }}
                                                            >
                                                                {formData.skills.map(skillId => {
                                                                    const skill = availableSkills.find(s => s._id === skillId);
                                                                    return skill ? (
                                                                        <span key={skillId} className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-xs font-medium flex items-center gap-1">
                                                                            {skill.name}
                                                                            <X size={12} className="hover:text-red-500 cursor-pointer" onClick={(e) => { e.stopPropagation(); toggleSkill(skillId); }} />
                                                                        </span>
                                                                    ) : null;
                                                                })}
                                                                <input
                                                                ...
                                                                <div className="ml-auto pointer-events-none">
                                                                    {isSkillDropdownOpen ? <ChevronUp size={16} className="text-gray-400" /> : <ChevronDown size={16} className="text-gray-400" />}
                                                                </div>
                                                            </div>

                                                            {/* Dropdown Menu */}
                                                            {isSkillDropdownOpen && (
                                                                <div className="absolute top-[102%] left-0 w-full bg-white border border-gray-100 rounded-lg shadow-xl z-20 max-h-60 overflow-y-auto animate-in fade-in zoom-in-95 duration-100">
                                                                    {filteredSkills.length === 0 ? (
                                                                        <div className="p-3 text-center text-gray-500 text-sm">No skills found. Add from master.</div>
                                                                    ) : (
                                                                        filteredSkills.map(skill => {
                                                                            ...
                                                                            return (
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    {skill.description && <span className="text-xs text-gray-400 hidden group-hover:inline max-w-[150px] truncate">{skill.description}</span>}
                                                                                </div>
                                                                            );
                                                                        })
                                                                    )}
